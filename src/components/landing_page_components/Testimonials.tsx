      ...
      <div className="grid gap-8 lg:grid-cols-3 md:grid-cols-2">
        {[
          ...
          { name: 'Chima’s Fixes', text: 'Happy I took the advice, this is the level of professionalism my clients want to see. Goes hand in hand with my skills. Love it!' },
            ...
            <Box key={testimonial.name} p={6} bg="#2c2c2e" border="1px" borderColor="#FFD282" borderRadius="3xl" shadow="md" textAlign="left" position="relative">
                <div className="flex">
                    <div className="ml-auto">
                    <Image src={testimonialImage} alt="testimonial image" className='inline-flex items-center justify-center' mb={4}  borderRadius="full" boxSize="50px" />
              
                    </div>
                </div>
                <Heading size="md" mb={4} color="white">{testimonial.name}</Heading>
              <Text color="gray.300" fontSize="sm">{testimonial.text}</Text>
            </Box>
        ))}
      </div>
    </div>
